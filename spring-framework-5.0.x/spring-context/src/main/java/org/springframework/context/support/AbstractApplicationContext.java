...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 初始化应用信息广播器，并放入"appliactionEventMulticaster"bean中
				// Multicaster:多播器
				/**
				 * 观察者模式
				 * 事件广播器负责把事件推送给监听器
				...
				//在所注册的bean中查找Listener bean，注册到消息广播器中
				/**
				 * 观察者模式
				 * 注册监听器
				 */
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		/**
		 * 这里创建BeanFactory 是 DefaultListableBeanFactory
		 * 并且加载beanDefinition
		 */
		refreshBeanFactory();
	...
	protected void initApplicationEventMulticaster() {
		...
		if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
			...
			/**
			 * 如果用户没自定义事件广播器，则使用默认的SimpleApplicationEventMulticaster
			 */
	...
	protected void finishRefresh() {
		...
		/**
		 * 启动所有继承了SmartLifecycle的bean
		 */
		getLifecycleProcessor().onRefresh();

		// Publish the final event.
		/**
		 * spring的ApplicationContext初始化完毕事件
		 * 还是让广播器来推送事件给监听器
		 */
