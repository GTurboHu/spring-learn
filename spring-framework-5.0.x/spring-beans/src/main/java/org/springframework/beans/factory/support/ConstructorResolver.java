...
class ConstructorResolver {
	...
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		//此方法被AbstractAutowireCapableBeanFactory.java的1307行autowireConstructor方法调用
		 ...
		 * bf.getBean("testA");这种不传参数
		 * bf.getBean("testA",args);这种是传参数的
		 */
		if (explicitArgs != null) {
			...
			if (argsToResolve != null) {
				//解析参数类型，如给定的方法的构造函数A(int,int)则通过此方法后就会把配置中的
				//("1","1")转换为(1,1)
				//缓存中的值可能是原始值也可能是最终值
		...
		if (constructorToUse == null) {
			...
			for (Constructor<?> candidate : candidates) {
				...
				if (constructorToUse != null && argsToUse.length > paramTypes.length) {
					...
					//如果已经找到选用的构造函数，或者，需要的参数个数小于当前的构造函数参数个数则终止，
					//因为已经按照参数个数降序排序
					break;
				}
				//minNrOfArgs是174行解析到的参数数量
				//paramTypes是反射获取的构造器
				...
				/**
				 * 配置文件中的参数有的是参数index的方式，有的是参数名称的方法
				 * 这里主要是按照参数名称找到参数
				 */
	...
	private int resolveConstructorArguments(String beanName, RootBeanDefinition mbd, BeanWrapper bw,
		...
		//此方法被autowireConstructor方法169行调用
